...
class Case1:
    ...
    def objf(x):
        ...
        x7 = x[:, 6]  # x0
        x8 = x[:, 7]  # y0
        x9 = x[:, 8]  # teta0 (x(9) in matlab)
        
        Cdx_t = torch.tensor(Case1.Cdx, device=x.device)
        Cdy_t = torch.tensor(Case1.Cdy, device=x.device)
        
        for i in range(len(Case1.Cdx)):
            teta2 = x[:, 9 + i]
            K1 = r1 / (r2 + 1e-12)
            K4 = r1 / (r3 + 1e-12)
    ...
    def cons(x):
        ...
        vo2 = angle_sequence_violation(x, num_angles=6)
